...
**Tool:** `gofusa` v0.19.0 (`github.com/SoundMatt/go-FuSa/cmd/gofusa`)  
...
| Tool version | v0.19.0 (pinned in CI) |
...
When gofusa is upgraded (e.g. v0.19.0 → vX.Y.Z):

1. Install new version in a separate environment
2. Run `gofusa check ./...` and compare results against v0.19.0 baseline
...
```yaml
run: go install github.com/SoundMatt/go-FuSa/cmd/gofusa@v0.19.0
```
