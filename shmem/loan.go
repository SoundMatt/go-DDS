...
// loaningPublisher wraps a shmPublisher and backs it with a BytePool to
// provide allocation-free loaned-sample publishing over the shmem transport.
...
// bufSize is the maximum sample size the pool will pre-allocate; pass 0 for
// the pool default (4096 bytes).
func NewLoaningPublisher(p dds.Participant, topic string, qos dds.QoS, bufSize int) (dds.LoaningPublisher, error) {
	...
	spub, ok := pub.(*shmPublisher)
	if !ok {
		_ = pub.Close()
		return nil, fmt.Errorf("shmem: %w: participant is not a shmem participant", dds.ErrLoanBuffer)
	}
...
func (lp *loaningPublisher) Loan(size int) ([]byte, error) {
	...
	if cap(buf) < size {
		lp.bp.Put(buf)
		return nil, fmt.Errorf("shmem: %w: requested %d, pool cap %d", dds.ErrLoanBuffer, size, cap(buf))
	}
	return buf[:size], nil
}
