...
func writeGCLatencyReport(t *testing.T, d gcLatResult) {
	...
	w("")
	w("For continuous monitoring, use the latmon command (lives in the")
	w("examples/ submodule, ROADMAP.md \"Architecture Initiative\" #71 Phase E):")
	w("")
	w("    cd examples && go run ./cmd/latmon --duration=0 --output=metrics.json")
	w("")
